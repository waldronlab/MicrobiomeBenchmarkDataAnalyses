...
# supp_tbl_s04_url <- "https://www.pnas.org/doi/suppl/10.1073/pnas.1002611107/suppl_file/st04.xlsx"
# supp_tbl_s04_file <- tempfile()
# download.file(url = supp_tbl_s04_url, destfile = supp_tbl_s04_file)
supp_tbl_s04_file <- "st04.xlsx"
...
Import ph annotations from bugphyzz (mean ph per genera)

```{r}
fname4 <- system.file(
  "extdata/ph_annotations_bugphyzz.tsv",
  package = "MicrobiomeBenchmarkDataAnalyses"
)

ph <- readr::read_tsv(fname4) %>% 
  magrittr::set_colnames(c("Genus", "pH"))
head(ph)
```
...
### pH enrichment (bugphyzz)

Run enrichment (ph):

```{r}
enrichment_ph <- createEnrichment(
    object = DA,
    priorKnowledge = taxTable2,
    enrichmentCol = "pH",
    namesCol = "Taxa",
...
```{r}
plotEnrichment(
    enrichment = enrichment_ph, 
    enrichmentCol = "pH",
    levels_to_plot = c("acidophile", "alkaliphile", "neutrophile")
)
```
...
positives_ph <- createPositives(
    ...
    alternative = "greater",
    verbose = FALSE,
    TP = list(c("DOWN Abundant", "acidophile"), c("UP Abundant", "neutrophile")),
    FP = list(c("DOWN Abundant", "neutrophile"), c("UP Abundant", "acidophile"))
)
head(positives_ph)
```
