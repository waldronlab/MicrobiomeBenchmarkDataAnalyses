...
## knit directory must be set to "project directory"
...
```{r prior knowledge}
biologicalInfo <- as.data.frame(rowData(tse)[,c("GENUS", "BIOSIS")])
biologicalInfo$newNames <- 
  paste0(rownames(biologicalInfo), "|", biologicalInfo$GENUS)
biologicalInfo$Type <- 
  ifelse(is.na(biologicalInfo$BIOSIS), "unknown", biologicalInfo$BIOSIS)
head(biologicalInfo)
```
...
sample_data(ps)[[grp]] <- relevel(sample_data(ps)[[grp]], contrast[1])
```
...
## Run normalization
ps <- quiet(runNormalizations(normalization_list = my_norm, object = ps))
```
...
```{r run DA methods}
DA <- quiet(runDA(method_list = my_methods, object = ps, weights = zinbweights))
```

Run ANCOMBC (separetely):
