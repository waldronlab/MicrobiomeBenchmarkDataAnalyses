...
```{r}
fname_recal <- paste0('./', dat_name, '_recal.rds')
if (!file.exists(fname_recal)) {
    DA_output_recal <- run_DA(
        object = ps_recal, conditions_col = conditions_col,
        conditions = conditions, verbose = FALSE 
    )
    saveRDS(DA_output_recal, fname_recal)
} else if (file.exists(fname_recal)) {
    DA_output_recal <- readRDS(fname_recal)
}
