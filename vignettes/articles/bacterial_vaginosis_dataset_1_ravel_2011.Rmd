...
path_to_sra_metadata <- "~/Projects/MicrobiomeBenchmarkDataAnalyses/MicrobiomeBenchmarkDataAnalyses/extra_files/SRA022855_run_metadata.csv"
...
# write.table(
#   abundance_matrix, 
#   file = "/home/samuelgamboa/Projects/MicrobiomeBenchmarkDataAnalyses/MicrobiomeBenchmarkDataAnalyses/inst/extdata/abundance_matrix_bacterial_vaginosis_dataset_1_ravel_2011.tsv",
...
Set list of DA methods included in the `benchdamic` package:
...
```{r}
fname1 <- system.file(
  "extdata/aer_bugphyzz.tsv", package = "MicrobiomeBenchmarkDataAnalyses"
)
...
```{r}
fname2 <- system.file(
  "extdata/gs_bugphyzz.tsv", package = "MicrobiomeBenchmarkDataAnalyses"
)
...
```{r}
fname3 <- system.file(
  "extdata/biosis.tsv", package = "MicrobiomeBenchmarkDataAnalyses"
)
...
## Run normalization
ps_genus <- 
  runNormalizations(normalization_list = my_norm, object = ps_genus) %>% 
  quiet()
```
...
Perform DA methods included in benchdamic:

```{r}
DA <- 
  runDA(method_list = my_methods, object = ps_genus, weights = zinbweights) %>%
  quiet()
```
...
```{r}
my_ancombc <-  ancombc(ps_genus, grp, grp)
...
```{r}
my_zinq <-  zinq(x = ps_genus, grp = grp, ref = contrast[2]) %>% 
  quiet()
DA$zinq <- my_zinq
```
