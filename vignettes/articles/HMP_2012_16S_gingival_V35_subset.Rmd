...
```{r}
## Add predefined normalization factors (as implemented in benchdamic)
ps <- runNormalizations(
    normalization_list = set_norm_list(), object = ps, verbose = FALSE
)

## Create matrix of weights (for edgeR, DESeq2, and limma only)
zinbWeights <- weights_ZINB(object = ps, design = grp)

## Run predefined DA methods (both implemented in benchdamic and added)
DA_methods <- set_DA_methods_list(grp, conditions)
DA_result <- runDA(
...
```{r}
DA_output <- run_DA(object = ps, conditions_col = grp, conditions = conditions)
```
