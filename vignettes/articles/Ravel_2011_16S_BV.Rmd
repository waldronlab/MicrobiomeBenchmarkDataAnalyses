...
## Import, summarzied by genus, and filter

Import the dataset from `MicrobiomeBenchmarkData`:
...
## This section if for selecting am equal number of samples per ethnicity group
...
select_samples <- row_names_list |> 
    map(~ sample(.x, min_n, replace = FALSE)) |>  
...
This sections mostly follows the benchdamic workflow. This includes other
methods not in benchdamic and modified plots.
...
## ALDEX2 throws an error, so it had to be removed
...
wilcox <- list(
    ...
    wilcox_clr = calcWilcox(data, 'CLR', log = FALSE),
    wilcox_relab_clr = calcWilcox(data, 'TSS + CLR', log = FALSE)
) |> 
    bind_rows(.id = 'method')
```
...
for (i in seq_along(transformations)) {
    mat <- assay(new_tse, transformations[i])
...
wilcox_raw |> 
    ...
    # mutate(
    #     value = case_when(
    #         transformation == 'CLR' | transformation == 'TSS + CLR' ~ exp(value),
    #         TRUE ~ value
    #     )
    # ) |> 
    mutate(value = log(value + 1)
        # value = case_when(
        #     transformation == 'TSS' | transformation == 'counts' ~ log(value + 1),
        #     TRUE ~ value
        # )
    ) |>
    ...
    labs(
        title = 'BV-associated bacteria identified (incorrectly) as UA by CLR',
        subtitle = 'CLR had an effect in both logFC (direction) and Wilcox test',
        y = 'Abundance value', y = 'Transformation method'
    )
## counts, TSS, CLR, TSS + CLR Add as Figure 
## Add 100
```
...
>It seems that the CLR transformation, made these two bacteria look as if they
were under-abundant in bacterial vaginosis (adjP <= 0.1). 
This could still be noted when the log of the CLR was not used. 
The fold change was really small, so probably establishing a threshold for
log FC would eliminate this, and the CLR would perform as well as the others.
...
calcLefse <- function(dat, assay) {
    ...
    dplyr::mutate(res, rawP = 0.05, adjP = 0.1)
...
taxa_annotations <-
        dplyr::distinct(dplyr::select(data, dplyr::starts_with('taxon')))
lefse <- list(
    lefse_counts = calcLefse(new_tse, 'counts'),
    lefse_relab = calcLefse(new_tse, 'TSS'),
    lefse_clr = calcLefse(new_tse, 'CLR'),
    lefse_relab_clr = calcLefse(new_tse, 'TSS + CLR')
)  |> 
    bind_rows(.id = 'method') |> 
    mutate(
        DA = ifelse(scores > 0, 'OA', 'UA')
    ) |> 
    left_join(taxa_annotations, by = c('Names' = 'taxon_name'))
...
```{r}
incorrect_taxa_lefse_clr <- wilcox_DA |> 
    dplyr::filter(
        method %in% c('wilcox_clr', 'wilcox_realb_clr'), DA == 'UA', 
        taxon_annotation == 'bv-associated'
    ) |> 
    pull(taxon_name)
...
> it seems that the problem with the direction is how LDA scores are calculated
when input is log transformed. Everything is solved when the log transformed
values are converted to exp.

## ZINQ


```{r}
calcZINQ <- function(dat, val_col, y_CorD, log = FALSE) {
    ...
    for (i in seq_along(pvalues)) {
        ...
        res <- tryCatch(
            error = function(e) NULL, {
                ZINQ::ZINQ_tests(
                    formula.logistic = formula_chr,
                    formula.quantile = formula_chr,
...
```{r}
calcZINQ(data, 'counts', y_CorD = 'D', log = FALSE)
```
