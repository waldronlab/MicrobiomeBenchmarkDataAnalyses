...
assays(new_tse)[[2]] <- apply(assays(new_tse)[[1]] + 1, 2, function(x) {
    x / sum(x)
})
names(assays(new_tse))[2] <- 'TSS'

assays(new_tse)[[3]] <- apply(assays(new_tse)[[1]] + 1, 2, function(x) {
    x / exp(mean(log(x)))
})
names(assays(new_tse))[3] <- 'CLR'
