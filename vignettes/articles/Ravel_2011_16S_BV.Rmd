...
tse_genus <- agglomerateByRank(
    tse_subset, rank = 'genus', na.rm = FALSE, onRankOnly = TRUE 
)
...
```{r}
## normalizations supported in benchdamic
ps <- runNormalizations(set_norm_list()[2:4], ps, verbose = FALSE)
zw <- weights_ZINB(ps, design = conditions_col)
DA_methods <- set_DA_methods_list(conditions_col, conditions)

## The following chunk of code is for compatibility with
## a more recent version of Seurat
...
calcWilcox <- function(dat, val_col, log = FALSE) {
    ...
    for (i in seq_along(log_fold_change)) {
        ...
        ## This doesn't work because I would be analyzing the geometric means
...
new_stats <- new_stats |> 
    ...
    mutate(
        Condition = case_when(
            ...
            Condition == "bacterial_vaginosis" ~ "VB"
        )
    )
```
